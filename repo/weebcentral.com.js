...
async detail(url) {
    ...
    const [title, cover, desc] = await Promise.all([
      this.querySelector(res, "div#top section section h1").text,
      this.querySelector(res, "div#top section section img").getAttributeText("src"),
      this.querySelector(res, "div#top section section section p").text,
    ]);

    const fullChapsUrl = await this.querySelector(res, "#chapter-list > button")?.getAttributeText("hx-get");

    const htmlChaplist = fullChapsUrl
      ? await this.request("", { headers: { "Miru-Url": fullChapsUrl } })
      : (await this.querySelector(res, "#chapter-list")).content;
    ...
    return {
      ...
      cover,
      desc,
      episodes: [{ title: "Chapters", urls: episodes }], // Shortened object creation
    };
  }
